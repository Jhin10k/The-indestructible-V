<template>
  <div>
    <el-table :data="goodslist" border style="width: 100%">
      ...
      <el-table-column label="商品价格" width="80px" prop="price">
      </el-table-column>
      <el-table-column label="市场价格" width="80px" prop="market_price">
      </el-table-column>
      <el-table-column label="图片">
        <template slot-scope="item">
          <div>
            <img
              class="imgInfo"
              :src="item.row.img ? $imgUrl + item.row.img : defaultImg"
              alt=""
            />
          </div>
        </template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      goodslist: "goodsmanage/getGoodsList",
      count: "goodsmanage/getCount",
      size: "goodsmanage/getSize",
    }),
  },
  methods: {
    //封装获取列表action
    ...mapActions({
      ...
      changePage: "goodsmanage/changePageAction",
    }),
    onChange(e) {
      //e 当前页码
      //触发修改页面的行动
